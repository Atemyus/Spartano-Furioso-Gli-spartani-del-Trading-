      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-cyan-600/20 to-blue-600/20 px-4 py-2 rounded-full border border-cyan-600/30 mb-6">
            ...
            <span className="text-cyan-400 text-sm font-medium">IL NOSTRO ARSENALE</span>
          </div>
          <h2 className="text-4xl md:text-5xl font-bold mb-6">
            <span className="text-white">LA </span>
            <span className="bg-gradient-to-r from-blue-500 to-cyan-500 bg-clip-text text-transparent">FALANGE</span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-6xl mx-auto relative z-10">
          {features.map((feature, index) => (
            ...
            >
              <div className={`relative overflow-hidden bg-gradient-to-br ${
              ...
              } border-2 border-cyan-600/30 rounded-2xl p-8 transition-all duration-500 transform hover:-translate-y-3 hover:scale-105 ${
                ...
                <div className={`relative w-20 h-20 bg-gradient-to-br ${feature.color} rounded-xl flex items-center justify-center mb-6 group-hover:scale-125 group-hover:rotate-6 transition-all duration-500 shadow-lg ${feature.glowColor}`}>
                  <div className="absolute inset-0 bg-cyan-400/20 blur-xl group-hover:blur-2xl transition-all"></div>
                  <feature.icon className="relative z-10 w-10 h-10 text-white drop-shadow-lg" />
                  <Flame className="absolute -top-2 -right-2 w-5 h-5 text-sky-500 opacity-0 group-hover:opacity-100 animate-fire transition-opacity" />
                </div>

                {/* Content with Animations */}
                <h3 className={`text-2xl font-black mb-2 ${
                  theme === 'dark' ? 'text-white' : 'text-black'
                } group-hover:text-cyan-500 transition-colors`}>{feature.title}</h3>
                <h4 className="text-cyan-500 font-bold mb-4 text-sm uppercase tracking-wider">{feature.subtitle}</h4>
                  ...
                  {feature.benefits.map((benefit, benefitIndex) => (
                    ...
                    >
                      ...
                      <span className={`text-sm font-medium ${
                        theme === 'dark' ? 'text-gray-300' : 'text-gray-700'
                      } group-hover/benefit:text-cyan-500 transition-colors`}>{benefit}</span>
                    </div>
                  ))}
                </div>
        ...
        <div className="mt-16 text-center">
          <div className="max-w-4xl mx-auto p-8 border-l-4 border-cyan-600 bg-gradient-to-r from-cyan-600/10 to-transparent rounded-r-lg">
            ...
            <cite className="text-cyan-500 font-medium">- L'Arsenale del Trader Spartano</cite>
          </div>
        </div>
      </div>
    </section>
